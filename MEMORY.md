...
- [2026-05-03] **UI-Design-System für Admin gesetzt.** Pilzling-CI 1:1 aus production-app übernommen (Cream + Dark + Orange + Blau-Dark + Akzent-Farben). CSS-Architektur gesplittet in `src/admin/assets/` (tokens, base, layout, components, admin als Hub). SSOT-Regel: keine Inline-Styles in Admin-PHP, keine page-spezifischen CSS-Files, alle Komponenten in `components.css` + `docs/DESIGN-SYSTEM.md`. Widget bleibt unabhängig (eigene Namespaces, inline in widget.js). Login + Dashboard als erste Pages auf das Standard-System umgestellt.
- [2026-05-03] **Design-System v2 — tiefgreifend erweitert.** Rubik-Font self-hosted (von production-app übernommen, DSGVO-freundlich). Format-Helper in `lib/helpers.php` (`formatDate`, `humanTimeDiff`) + JS-Pendant `src/admin/assets/format.js` (`AppFormat.*`) als SYNC-PAIR. Tabellen-Spalten-Standards (`.col-stars`, `.col-id`, `.col-status`, `.col-datum`, `.col-source`, `.col-shop`, `.col-author`, `.col-content`, `.col-product`, `.col-actions`) + `.id-cell` für muted ID-Rendering. Status-Mapping-Tabelle in DESIGN-SYSTEM.md (visibility/source/external_status/sync-runs.status → chip-Klassen). Sprach-/Schreibweise-Konventionen: Umlaut-Pflicht hart, Genderneutralität (`:innen`), Du-Form intern / Sie-Form Widget, Tone (sachlich, kompakt, lösungsorientiert). Empty-States 3-Typen-Pattern (wartend / leer-durch-Filter / echt-leer).
- [2026-05-03] **ASCII-Drift-Korrektur in CLAUDE.md/DESIGN-SYSTEM.md präzisiert.** Mein wiederkehrender Fehler beim Schreiben der ersten PHP-Files: ASCII-Substitutionen (`fuer`, `moeglich`, `Stueck` etc.) in UI-Strings + Code-Kommentaren — falsche Konvention-Interpretation. Korrekte Regel jetzt eindeutig formuliert mit Test-Frage für Grenzfälle: *"Wenn der Text theoretisch von einem Menschen gelesen werden könnte → Umlaute. Wenn der Text als String vom System interpretiert wird → ASCII."* ASCII gilt nur für technische Identifier (Variablen, Funktionen, DB-Spalten, Datei-Namen, CSS-Klassen, ENV, JSON-Keys, URL-Parameter). Alles andere (HTML-Output, UI-Labels, Code-Kommentare, Doku) bekommt Umlaute. Substitutions-Tabelle vollständig in DESIGN-SYSTEM.md Sektion 5b.
- [2026-05-04] **Pilzling-Go-Live-Aufbau komplett (Nacht-Build).** Master-Plan `2026-05-04-pilzling-go-live.md` durchgezogen — Phasen A-J in einer Sitzung. Inkludiert: Schema v2 (theme_overrides, feedback_url_slug, notification_emails), Widget-Komplett-Refactor (Aggregat, Avatar, Datum, Plattform-Icon, Pfeil-Nav, CTA, Footer mit Sporen-Icon — orientiert am Original-Sternfänger-Layout aber mit Sporeprint-Spin), Admin-Pages reviews/replies/analytics/widget-config/qr/shops/settings, public/feedback.php Bewertungs-Landing-Page, QR-Code via QuickChart.io (kein PHP-Lib-Install nötig). Alle Hub-Tile-Links funktionieren. Schema v2 wartet noch auf phpMyAdmin-Einspielung. Komplettes Pilzling-Production-Bereit-System mit Mock-Daten — echte API-Anbindung Phase K wartet auf Google + Trustpilot Freigaben.
- [2026-05-03] **Widget-Theming als 2-stufige Strategie dokumentiert.** Default-Mapping aus `shops.ci_primary` → automatisch (`--sp-accent` und Co.) PLUS Per-Shop-Overrides via `widget_configs.theme_overrides` JSON (Phase 2.5 Schema-Erweiterung + Phase 3 Konfigurator-UI). Begründung: 80%-Lösung über Auto-Mapping, manuelle Korrektur wo's nicht passt (z.B. CI-Farbe zu hell für Sporen-Visibility). User-Wunsch — klassisches CI-Controlling-Tool. Widget-Code in `widget.js` ist bereits vorbereitet: `applyShopBranding()` liest `theme_overrides` falls vorhanden, `THEME_OVERRIDE_MAP` definiert Override-Keys → CSS-Variablen-Mapping (erweiterbar ohne DB-Migration). Phase 2.5 als neuer Roadmap-Eintrag zwischen Phase 2 und 3 angelegt.
- [2026-05-03] **Architektur-Unterschied Sporeprint vs. production-app dokumentiert.** production-app = Sub-Dashboard-Layout mit View-Tabs intern (Sektion 11b im production-app-DESIGN-SYSTEM). Sporeprint = flacher Hub mit eigenen Sub-Pages pro Funktionsbereich. Konsequenz: production-app's `.action-tile` (Outline) passt nicht für Sporeprints zentrale Top-Level-Navigation — Sporeprint nutzt eigene `.hub-tile` (filled Blau-Dark, prominenter, größeres visuelles Gewicht). Lehre: Pattern-Quelle production-app dient als Inspiration, aber bewusste Abweichungen wo Architektur unterschiedlich ist. Niemals blind übernehmen.
- [2026-05-03] **CI-Refactoring (Phase 1.9) abgeschlossen.** Sechs-Phasen-Plan durchgezogen: Konvention präzisiert, alle UI-Strings + Code-Kommentare auf Umlaute, Card-Stub-Badge dezent (italic, kein Border/Background), Widget-Test nach `admin/widget-test.php` verschoben (hinter Verzeichnisschutz, eigene Inline-CSS raus), Public-Stub mit Sync-Pflicht-Kommentar konsolidiert, DESIGN-SYSTEM.md neue Sektion 12 "Reference-Implementation" mit drei Page-Skeletten zum 1:1-Kopieren. Verifikations-Greps: 0 ASCII-Substitutionen, 0 Inline-Style in Admin, 0 hardcoded Hex-Farben in Admin-PHP, 0 `new PDO` außerhalb config/database.php, 0 `echo json_encode` außerhalb lib/. SSOT-Konformität durchgängig.
